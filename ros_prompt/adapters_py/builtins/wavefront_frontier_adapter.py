...
class WaypointFollowerTest(Node):
    ...
    def __init__(self):
        ...
        odom_qos = QoSProfile(
            ...
            durability=QoSDurabilityPolicy.VOLATILE,   # ← match the publisher
        )
        ...
        self.model_pose_sub = self.create_subscription(Odometry,
                                                       '/odom', self.poseCallback, odom_qos)
        ...
        self.costmap = None
        self.exploration_complete = False   # ← new flag
    ...
    def _on_goal_response(self, future):
        ...
        if not goal_handle.accepted:
            self.error_msg("FollowWaypoints goal was rejected")
            return

        self.info_msg("Goal accepted by server")
    ...
    def _on_result(self, future):
        try:
            ...
            self.info_msg(f"Waypoints finished with status {status}")
        except Exception as e:
            self.error_msg(f"Result call failed: {e}")
            return

        # Launch next frontier after previous finishes
        if self.exploration_complete:
            return                      # don't spawn another goal
        self.moveToFrontiers()          # continue searching
    ...
    def moveToFrontiers(self):
        ...
        self.info_msg("Sending FollowWaypoints goal")
        future = self.action_client.send_goal_async(action_request)

        # when goal handle arrives, chain to result future
    ...
    def run(self, block):
        ...
        self.info_msg('run: Sending goal request...')
...
# ---------------------------------------------------------------
# Significant changes for frontier_adapter.py
# ---------------------------------------------------------------
import py_trees
import rclpy, threading, time
from rclpy.executors import SingleThreadedExecutor

class FrontierExplorationAdapter(py_trees.behaviour.Behaviour):
    """
    Behaviour that starts WaypointFollowerTest in a background executor.

    • initialise():  create node, wait for map+pose, start exploration
    • update():      always RUNNING (mapping is continuous)
    • terminate():   shuts down executor and node
    """

    def __init__(self, node, name="FrontierExplorationAdapter", **kwargs):
        super().__init__(name)
        self.executor   = None
        self.worker     = None
        self.get_logger  = node.get_logger

        # optional params for initial pose / retries (with defaults)
        self.starting_pose = kwargs.get("starting_pose", [-2.0, -0.5])
        self.retries       = int(kwargs.get("retries", 2))

    # ------------------------------------------------------------
    def initialise(self):
        # 1) Ensure rclpy.initialise() once at process start
        if not rclpy.ok():          # works on Humble, Foxy, etc.
            rclpy.init()

        # 2) Build the node from original code
        self.worker   = WaypointFollowerTest()
        ...
        # 4) Wait (non-blocking) for pose + costmap once per tick
        self._setup_complete = False
        self._start_time     = time.time()

    # ------------------------------------------------------------
    def update(self):
        # Wait until base pose + costmap are ready (similar to original main())
        if not self._setup_complete:
            if self.worker.initial_pose_received and self.worker.costmap:
                self.worker.moveToFrontiers()
                self._setup_complete = True
                self.get_logger().info("Frontier exploration started")
            elif time.time() - self._start_time > 10.0:   # 10-s safety timeout
                self.get_logger().error("Timed-out waiting for pose/map")
                return py_trees.common.Status.FAILURE
            self.get_logger().info("Waiting for initial pose and costmap...")
            return py_trees.common.Status.RUNNING

        # once running, report SUCCESS when worker says it's done -----
        if self.worker.exploration_complete:
            self.get_logger().info("Frontier exploration finished")
            return py_trees.common.Status.SUCCESS
        
        self.get_logger().info("Frontier exploration ongoing...")
        return py_trees.common.Status.RUNNING
